...
const handleCoreScan = async (req, res, user) => {
  const { url } = req.body || {};
...
const handleEeatScan = async (req, res, user) => {
  const { url } = req.body || {};
  const targetUrl = normalizeUrl(url);
...
export default async (req, res) => {
  ...
  try {
    ...
    if (routeParts.length === 0) {
      return await handleCoreScan(req, res, user);
    }

    if (routeParts.length === 1 && routeParts[0] === 'eeat') {
      return await handleEeatScan(req, res, user);
    }
